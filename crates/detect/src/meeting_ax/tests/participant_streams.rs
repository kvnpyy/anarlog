...
#[test]
fn test_zoom_native_speaker_view_requires_dominant_tile_and_unmuted_roster_row() {
    let mut self_tile = fixture_node(
        30,
        "AXButton",
        "video item ANLG Linux Native Zoom QA Active Speaker",
        &[11, 2],
    );
    self_tile.bounds = Some(AxRect {
        x: 501.0,
        y: 125.0,
        width: 200.0,
        height: 113.0,
    });
    self_tile.within_zoom_meeting_scope = true;
    let mut speaker_tile = fixture_node(
        31,
        "AXButton",
        "video item John Jeong Active Speaker",
        &[11, 3],
    );
    speaker_tile.bounds = Some(AxRect {
        x: 119.0,
        y: 244.0,
        width: 949.0,
        height: 534.0,
    });
    speaker_tile.within_zoom_meeting_scope = true;
    let mut roster_name = fixture_node(32, "AXStaticText", "John Jeong", &[11, 59, 9, 1, 5]);
    roster_name.within_zoom_meeting_scope = true;
    let mut unmuted = fixture_node(33, "AXButton", "Unmuted", &[11, 59, 9, 1, 21]);
    unmuted.bounds = Some(AxRect {
        x: 1530.0,
        y: 608.0,
        width: 24.0,
        height: 24.0,
    });
    unmuted.within_zoom_meeting_scope = true;

    let nodes = [self_tile, speaker_tile, roster_name, unmuted];
    let streams = find_participant_streams(&MeetingPlatform::Zoom, &MeetingSurface::Native, &nodes);

    assert_eq!(streams.len(), 1);
    assert_eq!(streams[0].participant_name.as_deref(), Some("John Jeong"));
    assert!(streams[0].is_active_speaker);
    assert!(
        streams[0]
            .signals
            .contains(&"dominant-speaker-view".to_string())
    );

    let mut muted = nodes.to_vec();
    muted[3].title = Some("Muted".to_string());
    muted[3].text = node_text(
        &muted[3].role,
        &muted[3].title,
        &muted[3].value,
        &muted[3].description,
    ...
    assert!(
        find_participant_streams(&MeetingPlatform::Zoom, &MeetingSurface::Native, &muted,)
            .is_empty()
    );

    let mut ambiguous = nodes.to_vec();
    ambiguous[0].bounds = ambiguous[1].bounds.clone();
    assert!(
        find_participant_streams(&MeetingPlatform::Zoom, &MeetingSurface::Native, &ambiguous,)
            .is_empty()
    );
}
