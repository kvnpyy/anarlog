...
Use Acorn's typed, read-only interfaces for meeting data. Do not use `find`,
`grep`, `rg`, filesystem crawling, or direct SQLite queries to find or read
meetings.

Prefer the Acorn MCP tools when they are available:
...
If MCP is unavailable, use the Acorn CLI with `--json`:
...
The CLI discovers Acorn's database from the platform application-data
